...
/**
 * Seeds the "Join the Mailing List" feature:
 *   1. A form (email + location + phone) in the form-builder `forms` collection.
 *   2. A published page at /mailing-list that renders the form (reuses the FormBlock).
 *   3. A "Mailing List" link in the Header nav.
 *
 * Idempotent — safe to run repeatedly. Submissions are forwarded to a Google Sheet
 * by src/hooks/forwardFormSubmissionToSheet.ts (filtered by the form title below).
 *
 * Run with: pnpm seed:mailing-list
 */

const FORM_TITLE = 'Join the Mailing List'
const PAGE_SLUG = 'mailing-list'
const NAV_LABEL = 'Mailing List'
...
const richText = (children: Array<ReturnType<typeof paragraphNode> | ReturnType<typeof headingNode>>) => ({
  root: {
    type: 'root' as const,
    children,
    direction: 'ltr' as const,
...
const run = async () => {
  ...
  try {
    ...
    if (existingForms.docs[0]) {
      form = await payload.update({
        collection: 'forms',
        id: existingForms.docs[0].id,
        data: formData,
        overrideAccess: true,
      })
      console.log(`✓ Updated form "${FORM_TITLE}" (${form.id})`)
    } else {
      form = await payload.create({ collection: 'forms', data: formData, overrideAccess: true })
      console.log(`✓ Created form "${FORM_TITLE}" (${form.id})`)
    }
    ...
    if (existingPage.docs[0]) {
      ...
      console.log(`✓ Updated page /${PAGE_SLUG} (${page.id})`)
    } else {
      ...
      console.log(`✓ Created page /${PAGE_SLUG} (${page.id})`)
    }
    ...
    if (alreadyLinked) {
      console.log('✓ Header nav already links to the Mailing List page')
    } else {
      ...
      console.log(`✓ Added "${NAV_LABEL}" to the Header nav`)
    }

    console.log('\n========================')
    console.log('✅ Mailing list is ready at /mailing-list')
    console.log('   Submissions are stored in Payload (Form Submissions) and forwarded')
    console.log('   to your Google Sheet when GOOGLE_SHEETS_WEBHOOK_URL is set.')
  } finally {
